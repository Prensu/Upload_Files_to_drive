    ...
    if (!empty($file_id)) {
        ...
        if (!empty($fileData)) {
            ...
            if (!empty($_SESSION['google_access_token'])) {
                ...
                $data = $GoogleDriveApi->GetAccessToken(
                    GOOGLE_CLIENT_ID,
                    REDIRECT_URI,
            ...
            if (!empty($access_token)) {
                try {
                    ...
                    $drive_file_id = $GoogleDriveApi->UploadFileToDrive(
                        $access_token,
                        $file_content,
                        $mime_type
                    );
                    ...
                    if ($drive_file_id) {
                        ...
                        $drive_file_meta = $GoogleDriveApi->UpdateFileMeta(
                            $access_token,
                            $drive_file_id,
                            $file_meta
                        );
                        ...
                        if ($drive_file_meta) {
                            ...
                            $statusMsg .= '<p><a href="https://drive.google.com/open?id=' .
                                $drive_file_meta['id'] .
                                '" target="_blank">' .
                                $drive_file_meta['name'] .
                                '</a></p>';
                        }
                    }
                } catch (Exception $e) {
